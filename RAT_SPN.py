...
class ScopeBasedParamProvider:
    ...
    def grab_sum_parameters(self, num_inputs, num_sums, name=None):
        num_vars = num_inputs * num_sums
        # print('using sum parameters [{}, {}]'.format(self.sum_params_used,
...
class ProductVector(NodeVector):
    ...
    def sample(self, inputs, seed=None):
        ...
        # TODO somehow check if this actually does what it is supposed to do
...
class SumVector(NodeVector):
    ...
    def sample(self, inputs, seed=None, differentiable=False):
        # print("SAMPLING FROM SUM VECTOR")
        # Single Sample of [1,16,9] (1,full scope size, num gaus^2)
        # print(inputs)
        # TODO is self weights really sum to 1? is it a prob?
        # It should not matter since we set it as logits
        # print("all weight?: ", self.weights)
        # print("weigts", self.weights[0])
        # print("own size:", self.size)
        # print("inputs? ", self.num_inputs)
        # TODO dafuq as that shape

        # print("inputs: ", inputs)
        # TODO Removes weird extra dimension, not sure if its neccesary
        ...
        print("inputs", sess.run(inputs))
        inputs = tf.concat(inputs, 2)
        print("concated", sess.run(inputs))
        inputs = tf.transpose(inputs, [0, 2, 1])
        print("transposed:", sess.run(inputs))
        logits = tf.transpose(self.weights[0])
        """
        ...
        # shape 10,20,2
        # stack them together so one thingy with index, sampled thing
        # full_idx = tf.transpose(full_idx, [1, 0, 2])
        # print(full_idx)
        ...
        # print(sess.run(case_idx))
        # print("full idx:", sess.run(full_idx))
        # print(sess.run(full_idx))
        # print("result: ", sess.run(result), result)
        # print("inputs:", sess.run(inputs), inputs)
        # exit()
...
class RatSpn(object):
    ...
    def sample(self, num_samples=10, seed=None):
        """
        print(self.vector_list[0][0].dist)
        exit()
        for el in self.vector_list:
            print(len(el))
            print(el)
            for vec in el:
                print(vec.scope)
        ...
        for layer_idx in range(1, len(self.vector_list)):
            ...
            for vector in self.vector_list[layer_idx]:
                # print("--------\n\n\n\n------")
                # print("vec scope:", vector.scope)
                # print("vec: ", vector)
                # print("inputs scope:", [vec.scope for vec in vector.inputs])
