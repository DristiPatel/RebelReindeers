...
"""
def build_nn_mnist(inp, output_shape, num_sum_weights, num_leaf_weights):
    batch_size = int(inp.shape[0])
    output_shape = list(output_shape)
    output_dims = 1
    for dim in output_shape[1:]:
    ...
    conv1 = tf.layers.conv2d(inputs=inp,
                             ...
                             padding='same',
                             activation=tf.nn.relu)
    linearized = tf.reshape(conv1, (batch_size, -1))
    linearized = tf.layers.dropout(linearized)
    sum_weights = tf.layers.dense(inputs=linearized,
                                  units=num_sum_weights,
                                  activation=None)
    output_proposal = tf.layers.dense(inputs=linearized,
                                      units=output_dims,
                                      activation=tf.nn.relu)
    output_proposal = tf.reshape(output_proposal, list(output_shape) + [1])
    tconv1 = tf.layers.conv2d_transpose(output_proposal,
                                        filters=16,
                                        kernel_size=3,
                                        padding='same')
    tconv1 = tf.nn.relu(tconv1)
    tconv2 = tf.layers.conv2d_transpose(inputs=tconv1,
                                        filters=num_leaf_weights,
                                        kernel_size=3,
                                        padding='same')
    leaf_weights = tf.reshape(tconv2, (batch_size, output_dims, num_leaf_weights))
    return sum_weights, leaf_weights
"""
...
def build_nn_celeb(inp, output_shape, num_sum_weights, num_leaf_weights):
    ...
    with tf.variable_scope('nn'):
        ...
        leaf_weights = tf.layers.dense(inputs=leaf_linear,
                                units=num_leaf_weights,
                                activation=None)
        #leaf_weights = tf.reshape(leaf2, (batch_size, output_dims, num_leaf_weights))
    print_num_vars()
